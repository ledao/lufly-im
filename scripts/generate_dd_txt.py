...
if __name__ == "__main__":
    ...
    for item in CharShapeTable.select():
        if item.char in char_to_phone:
            print(f"M: {item.char} multiply shapes.")
            continue
        else:
            char_to_phone[item.char] = item.shapes
    print(f"total {len(char_to_phone)} char shapes")
    ...
    for item in CharPhoneTable.select():
        ...
        if item.char not in char_to_phone:
            print(f"A: {item.char} has no shapes in charphonetable")
            continue
        else:
            items[f"{item.char}\t{item.phones+char_to_phone[item.char]}"] = 40000
    ...
    for item in WordPhoneTable.select():
        ...
        if first_char not in char_to_phone:
            print(f"A: {first_char} has no shapes in wordphonetable")
            continue
        elif last_char not in char_to_phone:
            print(f"A: {last_char} has no shapes in wordshapetable")
            continue
        else:
            items[f"{item.word}\t{item.phones+char_to_phone[first_char]+char_to_phone[last_char]}"] = 20000
