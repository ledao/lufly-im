...
if __name__ == "__main__":
    ...
    fname, output_dir = sys.argv[0], "rime_xiaolu"
    if not Path(output_dir).exists():
        os.makedirs(output_dir)
    version = datetime.datetime.now().strftime('%Y%m%d.%H%M%S')

    with open(output_dir + "/xiaolu.schema.yaml", 'w', encoding='utf8') as fout:
        fout.write(f"# 小鹭输入法\n")
        ...
        fout.write("\nschema:\n")
        fout.write("  schema_id: xiaolu\n")
        fout.write("  name: 小鹭输入法\n")
    ...
    with open(output_dir + "/xiaolu.dict.yaml", 'w', encoding='utf8') as fout:
        fout.write("# xiaolu dictionary\n")
        fout.write("# encoding: utf-8\n")
        fout.write("# \n")
        fout.write("# 鹭音形输入法码表\n")
        fout.write("# 机器生成，请勿修改\n")
        ...
        for item in generate_single_chars(char_to_shape, LU_SP_SCHEMA):
        ...
        for item in generate_full_words(char_to_shape, LU_SP_SCHEMA):
            fout.write(f"{item.decode}\t{item.encode[0:-2]}\n")
            fout.write(f"{item.decode}\t{item.encode[0:-1]}\n")
            fout.write(f"{item.decode}\t{item.encode}\n")

    with open(output_dir + "/xiaolu.custom.yaml", 'w', encoding='utf8') as fout:
        fout.write("# xiaolu custom config\n")
        fout.write("# encoding: utf-8\n")
        fout.write("# \n")
        fout.write("# 鹭音形输入方案\n")
        fout.write("# 机器生成，请勿修改\n")
