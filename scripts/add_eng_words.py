...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        print("words format:english_word")
        sys.exit(1)

    _, words_path = sys.argv

    exist_wordphones = pipe(EngWordTable.select(), map(lambda e: e.word), set)
    ...
    with open(words_path, "r", encoding='utf8') as fin:
        ...
        to_add_words = pipe(
            ...
            map(lambda e: EngWordTable(
                word=e, priority=1, updatedt=datetime.now())),
        )
