...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 1:
        print("USAGE: python3 generate_dd_txt.py ", file=sys.stderr)
        sys.exit(1)

    output_dir = "lu_phone_xhe_shape/"
    ...
    char_to_shape = pipe(CharShapeTable.select(),
                         ...
                         reduceby(lambda e: e[0], lambda e1, e2: e2),
                         valmap(lambda e: e[1]),
                         dict
                         )
    ...
    with open(sys_single_char_data, 'w', encoding='utf8') as fout:
        ...
        pipe(
            ...
            map(
                lambda e: f"{e.char}\t{e.lu+char_to_shape[e.char]}#序40000"),
    ...
    with open(sys_word_data, 'w', encoding='utf8') as fout:
        ...
        pipe(
            ...
            map(lambda e: (f'{e.word}\t{e.lu}', e.word[0], e.word[-1])),
